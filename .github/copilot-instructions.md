# Copilot / agent instructions — MAUI Day companion app

Guidance for AI agents (and humans) working in this repo. This is the canonical
instructions file (repo-wide `.github/copilot-instructions.md`, also mirrored to
agents via the root `AGENTS.md` pointer). Read this first, then dive into the specific
files it points at. Keep it up to date when architecture or conventions change.
